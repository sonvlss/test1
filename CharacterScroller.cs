...
public class CharacterScroller : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!charData.IsUnlocked)
        {
            ...
            unlockButton.gameObject.SetActive(CoinManager_BR.Instance.Coins >= charData.price);
            lockButton.gameObject.SetActive(CoinManager_BR.Instance.Coins < charData.price);
        }
        else
        {
    ...
    IEnumerator LoadGame()
    {
        ...
        // chờ load gần xong (Unity load ngầm)
        while (asyncLoad.progress < 0.9f)
        {
            yield return null;
        }

        // kích hoạt ngay khi load đủ
        asyncLoad.allowSceneActivation = true;
    }

    public void WatchAdToGetCoin()
    {
        // Gọi ShowRewardedAd trong AdManager
        AdManager.Instance.ShowRewardedAd();
        AdManager.Instance.OnRewardEarned += AddCoinReward; // đăng ký callback
    }

    private void AddCoinReward()
    {
        // Cộng coin khi xem xong quảng cáo
        CoinManager_BR.Instance.AddCoins(coinRewardPerAd);

        Debug.Log("Nhận " + coinRewardPerAd + " coin sau khi xem quảng cáo!");

        // Cập nhật UI
        totalCoins.text = CoinManager_BR.Instance.Coins.ToString();

        // Hủy đăng ký callback để tránh cộng nhiều lần
        AdManager.Instance.OnRewardEarned -= AddCoinReward;
    }
}
