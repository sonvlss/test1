...
public class ParentPlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        if ((ScoreManager_BR.Instance.Score != 0) && (ScoreManager_BR.Instance.Score % updateValueScore == 0) && check && gameManager.GameState_BR != GameState_BR.GameOver)
        {
            check = false;
    ...
    IEnumerator MovingParentPlayer()
    {
        while (true)
        {
            if (playerController.isPlayerRunning && !playerController.isBlockedByBranch && !playerController.isRotatingTrunk && !playerController.touchDisable && gameManager.GameState_BR != GameState_BR.GameOver)
            {
